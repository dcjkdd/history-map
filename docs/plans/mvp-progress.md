...
发生冲突时，按 [ADR-0002](../decisions/0002-地形优先二期产品纠偏.md) 的顺序判断：用户当前要求、ADR-0002、二期范围/验收/任务、第一阶段 MVP 历史文档、ADR-0001 与长期规划。
...
## 14. 2026-08-02 产品纠偏决定

- 产品负责人复核最初需求与当前真实产品后确认：地形、山河、平原、关隘、基本方位、距离和军事通道本来就是产品核心，不能用缺少这些信息的二维点线版验证其价值。
- 因此，MVP-01—MVP-11 统一定位为“二维技术原型”。其代码、内容行级审核、自动门禁、部署和浏览器证据继续有效，但不能自动推导最终产品验收通过。
- MVP-11 原最终三方签字保持暂停；既有行级 `APPROVED` 不撤销，也不把范围外 `PENDING_*` / `REJECTED` 自动批准或删除。
...
- 正式候选地形只提交区域派生：80 个资产、78 张 z5—9 Terrarium 瓦片、`9,572,150 bytes`；15 个原始 COG 共 `79,648,961 bytes`，只在 manifest 记录 SHA，不进入仓库。详细来源、许可、处理和 SHA 见 [资产记录](../data/phase2-02-terrain-assets.md)。
- 默认运行层为俯视分层设色 + hillshade，并新增现代河南/陕西边界和“今”标签、比例尺、指北、完整 Copernicus/geoBoundaries/Natural Earth/历史不确定性署名。没有加入 3D，也没有新增 PHASE2-03 平原/通道/古道几何或历史作用 Claim。
- 地形错误会进入明确 `degraded` 状态并保留历史图层；重试执行 `setStyle -> style.load`，测试确认 source/layer 幂等、图层开关/事件/地点状态恢复、监听器清理和单一 Map。
- 自动测试已扩展到 19 个文件、120 项并通过；根与 `/history-map/` 构建均已生成，worker 为单文件且 0 个相对依赖，源目录和两套构建后的地形闭包校验通过。最终完整 `check` 通过。
...
- 用户于 2026-08-04 明确批准现代河流流向、现代低地标签、两点解释性通道和潼关三条短说明四组候选；资料笔记和审核表逐项保留来源、几何处理、边界、审核人及日期。
- 正式 JSON 与 lockfile 未修改；原有关中通道 Geography 的 `PENDING_SOURCE`、对应 Claim 的 `PENDING_REVIEW` 以及黄河/秦岭历史作用候选的 `REJECTED` 保持不变。5 个 `DISPUTED` 代表点、3 个 Geography、3 个 `INFERENCE / LOW` RouteSegment 和 6 个 `APPROXIMATE` 事件保持不变。
- 新显示 source 只在运行时从既有正式几何派生：黄河/渭河流向箭头、`[109.68, 34.40]` 低地标签锚点、反向复用 `route-tang-advance-01` 的两点通道及 `22px` 屏幕带；没有新增历史 Polygon、古道、正式路线或真实宽度 Claim。
- 潼关继续使用正式 `PASS / DISPUTED / LOW` 代表点和不确定性圆，改用与城市和战场形状不同的菱形符号；选择潼关后显示三条既有已批准短说明。
- 新增内容门禁、图层顺序/幂等、四组开关恢复、流向与路线区分、PASS 语义、双 base/worker/地形闭包测试；完整自动测试为 19 个文件、121 项。
- 根路径和 `/history-map/` 各自在 `1024×768`、`1440×900` 的 Codex 应用内浏览器复核：地形正常、单一 Map/Canvas、无横向 overflow、console 0 warning/error，署名可展开且与 overlay 矩形交集为 0。另验证事件状态下水系/地理关闭与恢复、潼关选择和三条短说明。
- 没有新增二进制或下载数据集；PHASE2-02 地形仍为 80 个资产、78 张瓦片和 `9,572,150 bytes`。新增政府网页只用于内容判断和署名链接，不被下载或打包；详细记录见 [PHASE2-03 军事地理展示与来源记录](../data/phase2-03-military-geography.md)。
- 上述浏览器证据只覆盖真实执行的两个尺寸和两个 base，不宣称完整浏览器品牌矩阵，也不代替第二期最终产品/内容签字。

## 17. PHASE2-04 实现与验证证据

- 开始时独立 worktree 为 detached `HEAD` 且干净；`HEAD`、本地 `master`、`origin/master` 与 GitHub `master` 均现场核对为 `57685ed7effc8575e89e859a287b517569a9a811`，父提交为 `fd8aa92f698e14180b15d727dc7533e9898c81a7`。仓库没有 `.codegraph/`，因此按规则跳过 CodeGraph。
- Node `24.18.0` / npm `11.16.0` 下，修改前 `npm --prefix frontend ci` 与完整 `check` 通过；正式 JSON SHA-256 为 `34927c09eb03f37e2b7d884afb34172dc9990bc3549a6c1638813ddc251beee6`，lockfile SHA-256 为 `d2aa04cb6893326b206629ad8f8b3e3db573d1632fc847451aa5db888e9c7f22`。
- 用户于 2026-08-04 首轮批准燕军/唐军方向、距离方法和结果、路线短说明及视觉区分；2026-08-06 二次批准资料笔记第 10.7 节三条燕军补充来源、候选第四句与禁止扩展边界。17 个候选均有唯一审核行、审核人和日期。
- 正式 JSON 与 lockfile 未修改；5 个 `DISPUTED` 代表点、3 个 Geography、3 个两点 `INFERENCE / LOW` RouteSegment 和 6 个 `APPROXIMATE` 事件保持不变。关中通道及历史作用候选的 `PENDING_*` / `REJECTED` 状态未改变。
- 运行时只复制正式两点路线几何，显示燕军向西、唐军向东的单个 `▶`、中文方向及 `120/80/55 km` 现代代表点间大圆距离。距离采用半径 `6371.0088 km` 的 Haversine 算法并取整到最近 `5 km`，固定声明不等于唐代道路或历史行军里程。
- 路线点击显示获批的四条短说明、距离方法、正式 Citation / Source；燕军另显示三条补充来源及禁止扩展边界。没有新增中间节点、道路、古道、渡口、攻关点、撤退线、速度、用时或正式 Claim。
- 完整 `check` 通过：20 个测试文件、128 项测试；内容门禁固定核对 17 个批准候选、算法/取整、正式 SHA 和审核状态。根与 `/history-map/` 构建、worker 和地形闭包通过；既有地形仍为 80 个文件、78 张瓦片、`9,572,150 bytes`，没有新增二进制或第三方数据集。
- Codex 应用内浏览器实际覆盖根路径和 `/history-map/` 的 `1024×768`、`1440×900`：正常地形 ready、1 个 Map、1 个 Canvas、console 0 warning/error，路线/河流箭头可区分，方向、距离、署名和 overlay 可读且未见水平 overflow。另验证燕军/唐军路线详情、事件回退隐藏、四组图层开关状态保留，以及一次受控本地资源缺失后的同一 Map `style.load` 恢复。
- 三条新网页只作为人工审核来源和可点击出处，不下载、打包或作为运行时依赖；详细来源、算法、验证与许可边界见 [PHASE2-04 路线记录](../data/phase2-04-route-direction-distance.md)。

## 18. PHASE2-05 实现与验证证据

- 开始与最终现场核对 `HEAD`、本地 `master`、`origin/master` 与 GitHub `master` 均为 `9a63d162f4a15ea882ab83ad5a10bb1b28a8542c`，远端没有漂移；仓库没有 `.codegraph/`，按规则跳过。
- 用户于 2026-08-11 明确批准方案 A。实现只重排现有页面：桌面地图/短解释主行、122.4 px 跨列紧凑时间轴、地图外工具条、互斥图层/图例，以及事件/地点/路线的原生按需引用区；没有进入 PHASE2-06。
- 事件默认 2 条、潼关 3 条、路线 4 条短结论；路线继续使用 PHASE2-04 的方向、四句说明和 `120/80/55 km`。新增“相关路线”按钮只提供稳定键盘选择入口，Canvas 路线点击与既有 Pinia 选择语义继续保留。
- 最终 `npm --prefix frontend run check` 通过：20 个测试文件、131 项测试；根与 `/history-map/` 构建、单 worker 闭包、80 个地形文件 / 78 张瓦片闭包均通过。主入口保留既有大于 500 kB 的构建提示，没有新增运行时外网或第三方资产。
- Codex 内置浏览器实测根路径和 `/history-map/` 的 1024×768 / 1440×900：文档无横向或纵向页面溢出、地形 ready、1 个 Map / Canvas、6 个时间轴位置、console 0 warning/error。两档地图区/首屏面积为 50.0% / 55.0%，画布/首屏面积为 36.9% / 42.6%，控件和署名相关矩形交集为 0。
