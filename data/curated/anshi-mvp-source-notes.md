...
- 状态：缩小版正式数据与 MVP-11 映射审计已通过；二维技术原型的最终产品签字因缺少地形解释力而暂停。现有已批准记录可作为第二期输入，但新增地形、边界、通道、距离和军事地理解释必须独立记录来源、许可、审核与不确定性，不能由既有批准自动覆盖
- 建立日期：2026-07-29
- 叙事切片：潼关防线、灵宝出战与长安失守
...
## 10. PHASE2-04 路线方向、距离与点击说明候选（两轮已批准，display-only 已实现）

本节建立并保留 PHASE2-04 的人工确认记录，不属于既有 MVP-11 正式 Source、Citation、Claim、RoutePlan 或 RouteSegment 表。候选 ID 使用 `PHASE2-04-*` 前缀，不能因既有 RoutePlan 或 PHASE2-03 display-only 展示已经批准而自动升级。第 10.6 节记录首轮批准后的停止门，第 10.8 节记录二次批准；只有两轮门均关闭后，获批方向、距离和短说明才允许作为 display-only 运行时派生，仍不得写入正式 JSON。

### 10.1 现场审计结论

- 既有 `APPROVED` RoutePlan、RouteSegment、Event 与 Citation 足以支持燕军“洛阳→陕州→潼关、向西推进”和唐军“潼关→灵宝、向东出关”的宏观方向；方向只按已批准节点顺序和正式两点几何表达，不新增道路、中间节点、渡口、攻关点或撤退线。
- 正式三段 RouteSegment 的两个端点逐项等于对应 Place 的现代代表点。五个 Place 继续为 `DISPUTED`，三段路线继续为 `INFERENCE / LOW`；既有审核明确排除了里程和速度，因此任何计算值都必须作为新的 DistanceMethod / display-only 候选独立批准。
- 唐军路线已有逐句可用的批准内容：主体与潼关→灵宝顺序、灵宝西原方向“南近山、北临河的狭窄通道”、潼关关隘语义，以及两个现代争议代表点直连而非道路/战场/撤退线的边界。
- 燕军路线已有批准内容只支持主体与洛阳→陕州→潼关宏观顺序、潼关关键防御节点和两段现代代表点直连的不确定性。现有批准不足以把整条燕军路线概括为“北侧黄河、南侧秦岭约束”或“沿崤函—潼关通道行军”；`claim-geography-yellow-river-role-modern-01` 与 `claim-geography-qinling-role-modern-01` 仍为 `REJECTED`，`geography-guanzhong-corridor` / 对应 Claim 仍为 `PENDING_SOURCE` / `PENDING_REVIEW`，PHASE2-03 的两点 `22px` 通道也只获批为 display-only 表达，均不能用来补这个事实缺口。
- PHASE2-03 的赭金通道带与 `route-tang-advance-01` 共用两个端点但方向相反：通道中心线按灵宝→潼关组织，唐军行动按潼关→灵宝组织。通道必须保持无方向的空间背景，不能让其名称或几何顺序替代军队行动方向。

### 10.2 方向与视觉语义候选

| phase2CandidateId | 候选展示 | 来源 / 输入 | 几何或显示处理 | 不确定性与禁止扩展 | 推荐决定 | 状态 |
|---|---|---|---|---|---|---|
| PHASE2-04-DIRECTION-YAN-01 | `燕军 · 向西`，两段沿 `fromPlaceId → toPlaceId` 显示路线方向 | `claim-route-yan-westward-plan-modern-01`；`claim-route-yan-westward-segment-01-inference-01`；`claim-route-yan-westward-segment-02-inference-01`；正式 `route-yan-westward-01/02` | 只读取既有坐标顺序：洛阳→陕州、陕州→潼关；不生成新几何 | 只表达已批准宏观节点顺序；不表示道路中心线、行军轨迹、速度、渡口或攻关位置 | 批准为路线 display-only 方向标签与箭头 | APPROVED |
| PHASE2-04-DIRECTION-TANG-01 | `唐军 · 向东`，沿 `fromPlaceId → toPlaceId` 显示出关方向 | `claim-route-tang-advance-plan-modern-01`；`claim-route-tang-advance-segment-01-inference-01`；正式 `route-tang-advance-01` | 只读取既有潼关→灵宝坐标顺序；不生成新几何 | 不扩展到陕州、洛阳或撤退线；不表示唐代道路、战场中心或部署 | 批准为路线 display-only 方向标签与箭头 | APPROVED |
| PHASE2-04-PRESENTATION-DIRECTION-SEMANTICS-01 | 路线、现代河流流向与解释性通道的区分规则 | 既有路线样式；PHASE2-03 `PHASE2-CLAIM-YELLOW-FLOW-01`、`PHASE2-CLAIM-WEI-FLOW-01`、`PHASE2-GEOMETRY-EAST-GUANZHONG-01` | 河流继续使用蓝色实线上的重复 `➤` 和“现代总体流向”图例；路线继续使用主体色虚线，并为每段增加路线色单个 `▶` 及中文方向标签；赭金 `22px` 通道不加箭头，只保留无方向的 display-only 背景。三者分别随水系、路线、山地/低地/通道开关控制 | 唐军路线颜色即使接近蓝色，也必须同时由虚线、单个 `▶`、`唐军 · 向东` 文字和“INFERENCE / LOW”图例解码；不得把河流箭头、通道名称或中心线顺序当作军队方向 | 批准这一视觉与语义分层；具体字号、偏移和碰撞仍须真实浏览器调试 | APPROVED |

### 10.3 距离计算方法与候选结果

#### PHASE2-04-METHOD-DISTANCE-01

- 输入：只读取正式 `frontend/public/data/anshi/mvp-v1.json` 中每个 RouteSegment 的 `fromPlaceId`、`toPlaceId` 和两端坐标，并断言它们逐值等于对应 Place 的正式代表点；不读取现代道路、不调用路由服务、不添加中间点。
- 坐标边界：输入顺序为经度、纬度，按现有 WGS84 / OGC:CRS84 代表点处理；灵宝来源仍保留“无法区分 WGS84 与 CGCS2000”的既有残余不确定性。所有端点都只是现代 `DISPUTED` 代表点。
- 地球模型与算法：采用半径 `R = 6371.0088 km` 的平均地球球体，用 Haversine 大圆距离计算两个端点的表面最短距离；不把经纬度差直接当公里，也不声称使用椭球测地线、道路中心线或地形坡面距离。
- 公式：`a = sin²(Δφ/2) + cos φ1 · cos φ2 · sin²(Δλ/2)`；`d = 2R · asin(min(1, √a))`。实现必须对输入范围和有限数值做校验，测试固定下表三组输入。
- 单位与取整：内部结果以公里保留足够精度；页面只显示四舍五入到最近 `5 km` 的整数，正数取整规则固定为 `floor(km / 5 + 0.5) × 5`。
- 页面措辞：每段显示 `现代代表点间直线距离约 N 公里`；方法说明固定为“按两个现代争议代表点的大圆距离计算，取整到最近 5 公里；不等于唐代道路或历史行军里程。”
- 禁止扩展：不得据此生成速度、用时、补给能力、道路长度、路线总里程或坐标精度结论；不得隐藏单段的代表点与方法边界。

| phase2CandidateId | RouteSegment | 输入坐标（经度、纬度） | Haversine 原始结果 | 候选页面值 | 证据与边界 | 推荐决定 | 状态 |
|---|---|---|---:|---|---|---|---|
| PHASE2-04-DISTANCE-YAN-01 | `route-yan-westward-01` 洛阳→陕州 | `[112.4545867, 34.6769987]` → `[111.1488645, 34.7915940]` | `119.995101 km` | `现代代表点间直线距离约 120 公里` | 两端来自已批准但 `DISPUTED` 的现代应天门展示地标与宝轮寺塔故城内地标；不是历史城市中心或道路里程 | 按 PHASE2-04-METHOD-DISTANCE-01 批准 display-only 结果 | APPROVED |
| PHASE2-04-DISTANCE-YAN-02 | `route-yan-westward-02` 陕州→潼关 | `[111.1488645, 34.7915940]` → `[110.2909781, 34.6035548]` | `81.167887 km` | `现代代表点间直线距离约 80 公里` | 两端来自已批准但 `DISPUTED` 的现代故城内地标与旧城遗址代表点；不是唐代治所、关城或攻关里程 | 同上 | APPROVED |
| PHASE2-04-DISTANCE-TANG-01 | `route-tang-advance-01` 潼关→灵宝 | `[110.2909781, 34.6035548]` → `[110.872607, 34.615468]` | `53.246055 km` | `现代代表点间直线距离约 55 公里` | 两端来自已批准但 `DISPUTED` 的现代旧城遗址与“稠桑原”同名居民点；不是唐代关城、战场或撤退线里程 | 同上 | APPROVED |

推荐批准方法与三项结果作为独立的 display-only 计算记录，不写入正式历史 JSON；运行时从正式坐标确定性派生，并由 PHASE2-04 内容门禁锁定输入、算法、取整、措辞和三项输出。若内容负责人更偏好最近 `10 km` 或 WGS84 椭球测地线，必须在实现前修改本候选并重新计算，不能在代码中静默替换。

`PHASE2-04-METHOD-DISTANCE-01` 已由内容负责人随三项结果一并批准，状态为 `APPROVED`；实现仍须等待第 10.6 节规定的二次批准门关闭。

### 10.4 路线点击短说明候选与来源缺口

| phase2CandidateId | 路线 | 候选 2—4 条短说明 | 逐句依据 | 不确定性与缺口 | 推荐决定 | 状态 |
|---|---|---|---|---|---|---|
| PHASE2-04-PRESENTATION-YAN-01 | 燕军向潼关方向推进 | 1. `主体与方向：燕军由洛阳经陕州向潼关方向西进。` 2. `相关关隘：潼关是陕郡以西、进入关中的关键防御节点。` 3. `不确定性：两段只连接现代争议代表点，表达宏观节点顺序，不是唐代道路或行军轨迹。` | 1：`claim-route-yan-westward-plan-modern-01`、`claim-event-02-summary-modern-01`；2：`claim-place-tongguan-strategic-role-modern-01`；3：两个已批准 RouteSegment Claim | 三句均不新增正式 Claim；没有批准证据支持把整条燕军路线写成北侧黄河、南侧秦岭约束 | 批准这三句本身，但不能据此声称 P-003 的完整地形约束说明已通过 | APPROVED |
| PHASE2-04-GAP-YAN-CONSTRAINT-01 | 燕军向潼关方向推进 | 目标缺口：需要一句可复核的北侧河流、南侧山地或东西通道约束说明 | 现有 `REJECTED` / `PENDING_*` 项不能使用；PHASE2-03 两点通道也不等于整条燕军路线证据 | 第 10.7 节已补充三个可公开复核来源并形成窄化候选；内容负责人已确认适用范围、年代边界和禁止扩展，不能把地图视觉印象写成事实 | PHASE2-04 display-only 来源缺口已关闭；不改变任何正式 PENDING/REJECTED 项 | APPROVED |
| PHASE2-04-PRESENTATION-TANG-01 | 唐军出关行动 | 1. `主体与方向：唐军由潼关出关，向东推进至灵宝西原方向。` 2. `地形约束：灵宝西原方向现有材料只支持南近山、北临黄河的狭窄通道这一相对关系。` 3. `相关关隘：潼关是本路线出发关隘；当前点只是现代旧城遗址代表点，唐代关城位置仍有争议。` 4. `不确定性：直线只连接两个现代争议代表点，不是唐代道路、战场坐标或撤退线。` | 1：`claim-route-tang-advance-plan-modern-01`、`claim-event-03-why-modern-01`；2：`claim-place-lingbao-scope-modern-01`、`claim-event-04-summary-modern-01`、`PHASE2-CLAIM-EAST-GUANZHONG-01`；3：正式 `place-tongguan.placeType=PASS`、`claim-place-tongguan-site-dispute-modern-01`、`claim-place-tongguan-coordinate-candidate-01`；4：`claim-route-tang-advance-segment-01-inference-01` | 只复用已批准文字和实体类型；不采用 `CIT-ANSHI25-CH09-P027-P042` 的部署/战术重建，不声称 22px 是通道宽度，不增加败退路线 | 批准四句作为 route-level display-only 压缩说明，完整 Citation 入口继续来自既有 RouteSegment、Event 与 Place | APPROVED |

### 10.5 推荐人工决定与停止边界

1. 推荐批准两组方向候选，只表达已批准节点顺序；不改 RouteSegment 几何。
2. 推荐批准 `PHASE2-04-METHOD-DISTANCE-01`、最近 `5 km` 取整规则和三项候选值；不把结果写入正式历史 JSON。
3. 推荐批准唐军四句和燕军前三句的 display-only 压缩说明；路线详情必须保留 `INFERENCE / LOW`、现代争议代表点和完整 Citation 入口。
4. 推荐批准河流重复蓝色 `➤`、路线主体色虚线 + 单个 `▶` + 中文方向文字、赭金无箭头通道的语义区分规则。
5. 不推荐批准燕军“北河、南山、通道约束”句。优先取得新来源并重新提交人工审核；在该缺口关闭或产品负责人明确接受降级结论前，不实现正式 PHASE2-04 路线点击内容，也不声称 P-003、M-005 或 PHASE2-04 完成。

### 10.6 首轮人工批准记录与二次停止门

- 2026-08-04，内容负责人 `banq` 明确回复：“批准当前 PHASE2-04 候选；继续补充燕军地形约束来源，补齐并再次批准前不实现。”
- 本次批准覆盖第 10.2 节两组方向与视觉语义规则、第 10.3 节 Haversine 方法及 `120 / 80 / 55 公里` 三项结果、第 10.4 节唐军四句和燕军前三句；这些候选状态记为 `APPROVED`。
- `PHASE2-04-GAP-YAN-CONSTRAINT-01` 在首轮批准时继续为 `PENDING_SOURCE`。后续只允许补充可公开复核的 Source/Citation、候选第四句、适用范围、许可与禁止扩展；不得用 `REJECTED` / `PENDING_*` 内容或 PHASE2-03 display-only 几何替代来源。
- 新来源与候选第四句必须再次提交内容负责人明确批准。第二次批准前，不实现任何面向用户的 PHASE2-04 方向、距离或路线点击内容，不修改正式 `mvp-v1.json`，不声称 P-003、M-005 或 PHASE2-04 完成。

### 10.7 燕军区域地形约束补充来源（2026-08-06 二次批准）

#### 10.7.1 检索与核对结论

- 2026-08-04 已完整打开并逐段核对下表三个公开 HTML。三门峡市人民政府网页明确把崤函古道界定为古代洛阳至潼关间道路的统称，并说明沿线因地理形成多个隘口；灵宝市人民政府网页把其中西段明确限定为陕州至潼关，并给出“南依崤山、北临黄河”的相对地形；《光明日报》文章由三门峡市文物局原局长等署名，补充崤山、秦岭/华山、黄河的区域关系，并明确崤函古道在唐代为长安—洛阳交通干线。
- 三份来源共同支持的是“路线节点所在崤函区域的地形与通行背景”，不是安史之乱燕军行军道路的直接证据。检索中没有取得能够证明燕军使用某条崤函古道支线、现存路基、驿站、渡口或关隘序列的直接史料，因此第四句必须停在区域解释层。
- 三份网页均未发现开放再分发许可；项目只保存稳定链接、网页内定位和少量事实归纳，不下载、打包或再分发正文、图片、版面或附件，不产生新增本地资产或资产 SHA-256。

#### 10.7.2 Source 与 Citation 候选

| phase2CandidateId | 类型 | 资料名与提供方 | 版本或稳定定位 | URL / 访问日期 | 许可与使用边界 | 候选支持内容 | 当前核对状态 | 状态 |
|---|---|---|---|---|---|---|---|---|
| PHASE2-04-SRC-XIAOHAN-SCOPE-01 | 地方政府公开网页 | 〈崤函古道入选2024年度“最受欢迎古道”〉；三门峡市人民政府，文章来源三门峡日报，信息来自三门峡市公路事业发展中心 | 发布时间 2024-12-16 08:22；正文第二、三段 | [三门峡市人民政府固定 HTML](https://www.smx.gov.cn/4036/616951008/1869004.html)；访问日期 2026-08-04 | 未发现开放再分发许可；只作范围、隘口与区域战略背景的少量归纳，不复制正文或图片，不把报道当作燕军具体道路证据 | 古代洛阳至潼关间道路体系的范围；沿线因地理形成多个隘口 | 网页标题、日期、来源、正文与政府站点主体已完整打开核对；2026-08-06 内容负责人批准使用边界 | APPROVED |
| PHASE2-04-CIT-XIAOHAN-SCOPE-01 | 候选 Citation | PHASE2-04-SRC-XIAOHAN-SCOPE-01 | 正文第二段“崤函古道是对古代洛阳至潼关间道路的统称”至“由于地理原因……形成……多个隘口” | 同上 | 只支持洛阳—潼关区域范围及地理形成隘口；不支持单一路线中心线、安史之乱行军支线、里程或中间节点 | 燕军第四句的区域范围与隘口背景 | 2026-08-06 内容负责人批准 | APPROVED |
| PHASE2-04-SRC-XIAOHAN-WEST-TERRAIN-01 | 地方政府公开网页 | 〈函谷关历史文化旅游区〉；灵宝市人民政府 | 发布日期 2026-01-16 15:31；正文“秦函关古道”及下一两段 | [灵宝市人民政府固定 HTML](https://www.lingbao.gov.cn/16031/616500000/1265843.html)；访问日期 2026-08-04 | 未发现开放再分发许可；页面兼有旅游宣传与遗址信息，只采用陕州—潼关范围、南山北河相对关系和考古工作概况；不采用传说、宣传修辞或具体战争列举，不复制正文或图片 | 陕州—潼关西段南依崤山、北临黄河；调查发现汉及唐宋时期路基层 | 网页标题、政府来源、日期及第 86—90 行正文已完整打开核对；2026-08-06 内容负责人批准使用边界 | APPROVED |
| PHASE2-04-CIT-XIAOHAN-WEST-TERRAIN-01 | 候选 Citation | PHASE2-04-SRC-XIAOHAN-WEST-TERRAIN-01 | “秦函关古道位于崤函古道西段，西起潼关，东至陕州”及“古道南依巍巍崤山、北临九曲黄河”段 | 同上 | 只支持陕州—潼关西段的相对地形，不把这一表述扩展为洛阳—陕州全段，不证明燕军走过该古道或已发现的任一路基 | 燕军第四句中严格限定的西段南山北河关系 | 2026-08-06 内容负责人批准 | APPROVED |
| PHASE2-04-SRC-XIAOHAN-TANG-CONTEXT-01 | 报纸公开网页 | 〈通古达今的崤函古道〉；《光明日报》/光明网；侯俊杰（三门峡市文物局原局长）、关婷（陕西师范大学出版总社副编审） | 《光明日报》2024-03-03 第 11 版；网页 2024-03-03 04:20；“具有丰富内涵的文化之路”“始终繁荣畅达的经济通道”小节 | [光明网固定 HTML](https://news.gmw.cn/2024-03/03/content_37180338.htm)；访问日期 2026-08-04 | 光明网页标示版权所有，未发现开放再分发许可；只作区域山河关系与唐代交通背景的交叉核对，不复制正文、图片或版面，不用文化报道重建道路 | 崤山西连秦岭/华山、北滨黄河并构成区域交通孔道；唐代崤函古道是两京交通干线 | 网页标题、报纸版次、日期、作者身份及相关小节已完整打开核对；2026-08-06 内容负责人批准使用边界 | APPROVED |
| PHASE2-04-CIT-XIAOHAN-TANG-CONTEXT-01 | 候选 Citation | PHASE2-04-SRC-XIAOHAN-TANG-CONTEXT-01 | “崤山西与秦岭、华山相连……北滨黄河，是崤函地区主要的交通孔道”段；“两周、两汉、隋、唐实行两京制时期……”段 | 同上 | 只证明区域地形与唐代交通背景；不证明燕军沿古道、驿站、漕运线或任一考古路基行军 | 年代适用性与区域山河关系的交叉核对 | 2026-08-06 内容负责人批准 | APPROVED |

#### 10.7.3 候选第四句、逐句映射与禁止扩展

| phase2CandidateId | 候选展示位置 | 候选文字 | 逐句依据 | 不确定性与禁止扩展 | 推荐决定 | 状态 |
|---|---|---|---|---|---|---|
| PHASE2-04-PRESENTATION-YAN-CONSTRAINT-01 | 燕军路线点击说明第 4 句 | `地形约束：洛阳—潼关的崤函区域受山地与黄河影响，沿线因地理形成多处隘口；其中陕州—潼关西段南依崤山、北临黄河。` | 前半句：PHASE2-04-CIT-XIAOHAN-SCOPE-01、PHASE2-04-CIT-XIAOHAN-TANG-CONTEXT-01；后半句：PHASE2-04-CIT-XIAOHAN-WEST-TERRAIN-01 | 这是区域背景，不证明燕军使用“崤函古道”中的具体道路、支线、现存/考古路基、驿站或渡口；不得新增函谷关、崤陵关、硖石关等路线节点，不把陕州—潼关的南山北河关系扩大到洛阳—陕州，不复原唐代精确河道或通行边界 | 批准为燕军路线第 4 条 display-only 说明，并与已批准三句、`INFERENCE / LOW` 及完整来源入口共同显示 | APPROVED |

#### 10.7.4 推荐二次人工决定

1. 推荐批准三个 Source 与 Citation 候选作为区域地形和唐代适用背景，不将它们转换为正式 `mvp-v1.json` Source/Citation；运行时只保留可点击署名链接和候选句，不依赖外网加载。
2. 推荐批准 `PHASE2-04-PRESENTATION-YAN-CONSTRAINT-01` 原文，严格保留“其中陕州—潼关西段”的范围限定；路线详情合并显示此前已批准三句与本句，共四句。
3. 推荐把 `PHASE2-04-GAP-YAN-CONSTRAINT-01` 从 `PENDING_SOURCE` 关闭为“来源已补齐并获人工批准”；该关闭只针对 PHASE2-04 的 route-level display-only 说明，不升级正式 `geography-guanzhong-corridor=PENDING_SOURCE`、对应 Claim `PENDING_REVIEW` 或两条 `REJECTED` 历史作用候选。
4. 不批准从这些来源新增路线几何、关隘节点、古道名称标签、考古路基、驿站、渡口、里程、速度或历史河道；也不把燕军宏观直线改称历史道路。

### 10.8 二次人工批准记录

- 2026-08-06，内容负责人 `banq` 明确回复：“批准第 10.7 节全部来源、候选第四句及禁止扩展边界”。
- 第 10.7 节三个 Source、三个 Citation、燕军第四句及其禁止扩展状态更新为 `APPROVED`；`PHASE2-04-GAP-YAN-CONSTRAINT-01` 只在 PHASE2-04 display-only 路线说明范围内关闭。
- 本次批准允许开始 PHASE2-04 实现，但不授权新增正式历史实体、古道几何、关隘节点、考古路基、驿站、渡口、里程、速度或历史河道，也不授权进入 PHASE2-05。
- 正式 `geography-guanzhong-corridor=PENDING_SOURCE`、对应 Claim `PENDING_REVIEW`、两条黄河/秦岭历史作用 `REJECTED` 候选及所有既有审核历史保持不变。
